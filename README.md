...
Run the app here: http://96.54.58.71:3838/poweROC-main/ROC_power_app/. Because simulations can be time-consuming (and can be disrupted by internet timeouts), and because the online version is currently hosted on a free version of Shiny server that limits the amount of concurrent users, it is recommended that users with access to R/RStudio instead download and run a local copy of the app. The steps to do so are simple:
