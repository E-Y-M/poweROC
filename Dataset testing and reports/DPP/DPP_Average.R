...
colloff_data = read.csv("./Dataset testing and reports/DPP/ColloffWadeStrangeData.csv") %>% 
    ...
    mutate(response_type = case_when(faceSelected == perpetrator & targetLabel == "present" ~ "correctID",
           ...
           cond = case_when(treatmentLabel == "nothing" ~ "Biased",
                            treatmentLabel == "concealment" ~ "Fair"))
...
colloff_fair_TP_filler = colloff_props %>% 
    filter(cond == "Fair" & targetLabel == "present" & response_type == "filler") %>% 
    summarize(total = sum(n))

colloff_fair_TP_miss = colloff_props %>% 
    filter(cond == "Fair" & targetLabel == "present" & response_type == "miss") %>% 
    summarize(total = sum(n))

colloff_fair_TA_rej = colloff_props %>% 
    filter(cond == "Fair" & targetLabel == "absent" & response_type == "correctRej") %>% 
    summarize(total = sum(n))
